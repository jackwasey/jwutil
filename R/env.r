...
#'
#' \code{x} and \code{table} are identical to \code{match}. Lookup is done based
#' on environment element names; contents are ignored. For \code{%ine%},
#' \code{x} is a vector, and \code{table} is the environment to search. For
#' \code{%eine}, both arguments are environments.
#' @keywords internal
