...
test_that("cache with date limited files", {
  ...
  # save something to cache
  saveToCache("jack", startDate = startDate, endDate = endDate, envir = mytestenv, cacheDir = cd)
  ...
  expect_true(exists("oliver", envir = mytestenv)) # we keep both the dated and undated var names.
  #expect_true(exists("oliver2010-02-01to2011-03-15", envir = mytestenv)) # we keep both the dated and undated var names.
  expect_true(file.exists(findCacheFilePath("oliver2010-02-01to2011-03-15", cacheDir = cd)))
  expect_true("oliver2010-02-01to2011-03-15.RData" %in% lsCache(cacheDir = cd))

  expect_true(isCached(getCacheVarDated("oliver", startDate, endDate), cacheDir = cd))

  expect_true(any(grepl(pattern = "oliver", x = list.files(findCacheDir(cacheDir = cd)))))
  ...
  expect_false(file.exists(findCacheFilePath(getCacheVarDated("jack", startDate, endDate))))
  expect_false(file.exists(findCacheFilePath(getCacheVarDated("oliver", startDate, endDate))))
