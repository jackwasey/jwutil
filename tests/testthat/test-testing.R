...
test_that("combinations of commutative arguments", {
  ...
  result <- expect_that_combine(sum(1,2), testthat::equals(3))
  ...
  result <- expect_that_combine(sum(1,2,3,4,5,6,7), testthat::equals(28)) # this runs 7! = 5040 tests!

})

test_that("throw error for ?every iteration", {
  result <- expect_that_combine(stop("a", "b"), testthat::throws_error())
})
